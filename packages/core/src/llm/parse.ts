...
export function parseLLMJson<T>(content: string): T {
  ...
  throw new Error(
    `Failed to parse LLM JSON output: could not find valid JSON in response. Content preview: ${content.slice(0, 200)}`,
  );
}
...
function findFirstStructureStart(content: string): number {
  const objIdx = content.indexOf("{");
  const arrIdx = content.indexOf("[");

  if (objIdx === -1 && arrIdx === -1) return -1;
