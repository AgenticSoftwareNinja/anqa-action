...
describe("createEmptyMetrics", () => {
  it("returns zero-value metrics when called with no arguments", () => {
    ...
    expect(metrics).toEqual<QualityMetrics>({
      passRate: 0,
  ...
  it("applies all overrides at once", () => {
    const overrides: QualityMetrics = {
      passRate: 1,
...
describe("formatRAGContext", () => {
  const makeResult = (content: string): RAGResult => ({
    id: "1",
    content,
    type: "pattern",
    similarity: 0.9,
...
describe("deriveAppName", () => {
  ...
  it("ignores path, query, and hash — uses hostname only", () => {
    expect(deriveAppName("https://staging.acme.dev/app/dashboard?debug=true#top")).toBe(
      "staging-acme-dev",
    );
  });
...
describe("parsePlaywrightReport", () => {
  ...
  const makeReport = (suites: unknown[]) => ({ suites });

  const makeSuite = (file: string, specs: unknown[]) => ({ file, specs });

  const makeSpec = (tests: unknown[]) => ({ tests });

  const makeTest = (
    status: string,
    results: Array<{ duration?: number; status?: string; error?: { message: string; stack?: string } }>,
  ) => ({ status, results });
  ...
  it("parses a single passing test", () => {
    ...
    const results = parsePlaywrightReport(report as Record<string, unknown>);
  ...
  it("maps 'unexpected' status to 'failed'", () => {
    ...
    const [result] = parsePlaywrightReport(report as Record<string, unknown>);
  ...
  it("maps 'expected' status to 'passed'", () => {
    ...
    expect(parsePlaywrightReport(report as Record<string, unknown>)[0].status).toBe("passed");
  });
  ...
  it("maps 'skipped' status to 'skipped'", () => {
    ...
    expect(parsePlaywrightReport(report as Record<string, unknown>)[0].status).toBe("skipped");
  });
  ...
  it("maps 'flaky' status to 'flaky'", () => {
    ...
    expect(parsePlaywrightReport(report as Record<string, unknown>)[0].status).toBe("flaky");
  });
  ...
  it("maps an unknown status to 'failed'", () => {
    ...
    expect(parsePlaywrightReport(report as Record<string, unknown>)[0].status).toBe("failed");
  });
  ...
  it("flattens tests across multiple specs in the same suite", () => {
    ...
    expect(parsePlaywrightReport(report as Record<string, unknown>)).toHaveLength(2);
  });
