...
describe("createLogger", () => {
  ...
  function getStdoutEntry(callIndex = 0): Record<string, unknown> {
    const raw = stdoutSpy.mock.calls[callIndex][0] as string;
    return JSON.parse(raw.trimEnd());
  }

  function getStderrEntry(callIndex = 0): Record<string, unknown> {
    const raw = stderrSpy.mock.calls[callIndex][0] as string;
    return JSON.parse(raw.trimEnd());
  }
