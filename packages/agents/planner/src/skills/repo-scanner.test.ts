...
describe("detectFramework via scanRepository", () => {
  it("detects next framework and typescript language with playwright present", async () => {
    mockReadFile.mockImplementation(async (path) => {
      if (String(path).endsWith("package.json")) return makePackageJson();
...
describe("detectCIConfig via scanRepository", () => {
  it("detects .github/workflows with yml files", async () => {
    mockAccess.mockImplementation(async (path) => {
      if (String(path).includes(".github/workflows")) return undefined;
      throw new Error("ENOENT");
    });

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    mockReaddir.mockImplementation(async (path): Promise<any> => {
    ...
    mockReadFile.mockImplementation(async (path) => {
      ...
      if (p.endsWith("ci.yml")) return "name: CI\njobs:\n  test:\n    run: pnpm test";
      if (p.endsWith("release.yml")) return "name: Release\njobs:\n  deploy:\n    run: pnpm deploy";
      throw new Error("ENOENT");
    });
  ...
  it("skips non-yml/yaml files in workflows directory", async () => {
    ...
    mockReadFile.mockImplementation(async (path) => {
      if (String(path).endsWith("ci.yml")) return "run: test";
      throw new Error("ENOENT");
    });
...
describe("discoverTests via scanRepository", () => {
  it("finds spec.ts files and extracts describe/it descriptions", async () => {
    ...
    mockReaddir.mockImplementation(async (path): Promise<any> => {
      const p = String(path);
      if (p === "/repo" || p === "/repo/") return [makeDirent("tests", false)];
      if (p.endsWith("/tests")) return [makeDirent("login.spec.ts")];
      return [];
    });
...
test('redirects after login', async () => {});
  ...
  it("skips node_modules directories during walk", async () => {
    ...
    mockReaddir.mockImplementation(async (path): Promise<any> => {
      ...
      if (p.endsWith("/src")) return [makeDirent("app.test.ts")];
  ...
  it("classifies files in /e2e/ directory as e2e type", async () => {
    ...
    mockReaddir.mockImplementation(async (path): Promise<any> => {
      const p = String(path);
      if (p === "/repo" || p === "/repo/") return [makeDirent("e2e", false)];
      if (p.endsWith("/e2e")) return [makeDirent("checkout.spec.ts")];
      return [];
    });
...
describe("detectAISetup via scanRepository", () => {
  it("detects claude setup when .claude directory exists", async () => {
    mockAccess.mockImplementation(async (path) => {
      if (String(path).endsWith("/.claude")) return undefined;
      throw new Error("ENOENT");
    });
  ...
  it("detects copilot when .github/copilot-instructions.md exists", async () => {
    mockAccess.mockImplementation(async (path) => {
      if (String(path).includes("copilot-instructions.md")) return undefined;
      throw new Error("ENOENT");
    });
  ...
  it("detects cursor when .cursorrules exists", async () => {
    mockAccess.mockImplementation(async (path) => {
      if (String(path).endsWith(".cursorrules")) return undefined;
      throw new Error("ENOENT");
    });
