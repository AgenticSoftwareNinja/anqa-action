...
export async function evaluateCoverage(
  ...
  if (existingTests.length === 0) {
    const flowCoverages: FlowCoverage[] = flows.map((flow) => ({
      flowId: flow.id,
      flowName: flow.name,
...
- matchedTests = list of test file paths that cover this flow (empty array for uncovered)`,
  ...
  const matches = parseLLMJson<LLMFlowMatch[]>(result.content);

  // Build a lookup map from flowId → LLM result
  const matchMap = new Map<string, LLMFlowMatch>(
    matches.map((m) => [m.flowId, m]),
  );
...
function computeSummary(flows: FlowCoverage[]): CoverageSummary {
...
export const coverageEvaluatorSkill: Skill = {
  name: "coverage-evaluator",
  description:
    "Map discovered flows against existing tests to produce coverage map",
  async execute(ctx: AgentContext, input: unknown): Promise<CoverageMap> {
    const { flows, existingTests } = input as {
      flows: TestFlow[];
      existingTests: ExistingTestFile[];
    };
    return evaluateCoverage(
      ...
      existingTests,
      ctx.llm,
      ctx.config.modelsConfig.planner,
    );
  },
};
