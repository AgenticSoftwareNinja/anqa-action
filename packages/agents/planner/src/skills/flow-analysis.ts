...
import type { TestFlow, TestStep, TestAssertion } from "@agentic-nqa/core";
import type { PageInventory } from "./app-discovery.js";

export const flowAnalysisSkill: Skill = {
  name: "flow-analysis",
  description: "Identify and prioritize critical user flows from page inventory",

  async execute(ctx: AgentContext, input: unknown): Promise<TestFlow[]> {
    const { inventory, targetAppName } = input as {
      inventory: PageInventory;
      targetAppName: string;
    };

    // Query RAG for known patterns about this app
    const priorKnowledge = await ctx.rag.search(
      `test flows for ${targetAppName}`,
      { type: "app", limit: 5 },
    );
    ...
    const ragContext = formatRAGContext(
      priorKnowledge,
      "Prior knowledge about this app",
    );
...
Return 5-20 flows depending on app complexity.`,
    ...
    const flows = parseLLMJson<TestFlow[]>(result.content);
    return flows;
  },
};

function buildAppDescription(inventory: PageInventory): string {
  const lines: string[] = [];
  ...
  for (const page of inventory.pages) {
    ...
    if (page.forms.length > 0) {
      lines.push(
        `Forms: ${page.forms.map((f) => `[${f.fields.join(", ")}]`).join("; ")}`,
      );
    }
