...
export const testScaffoldingSkill = {
    name: "test-scaffolding",
    description: "Generate Playwright test code from a test flow",
    async execute(ctx, input) {
        const { flow, targetApp, baseUrl, authConfig } = input;
        // Retrieve relevant test patterns from RAG
        const patterns = await ctx.rag.search(`playwright test pattern for ${flow.name} ${flow.description}`, { type: "pattern", limit: 3 });
        const patternContext = formatRAGContext(patterns, "Relevant test patterns from knowledge base");
...
function toFileName(flowName) {
    return (flowName
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "") + ".spec.ts");
}
//# sourceMappingURL=test-scaffolding.js.map