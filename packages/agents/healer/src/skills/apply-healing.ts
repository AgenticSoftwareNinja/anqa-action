import type { AgentContext, Skill, HealingReport } from "@agentic-nqa/core";

export const applyHealingSkill: Skill = {
  name: "apply-healing",
  description: "Apply a healing fix to a test file",

  async execute(ctx: AgentContext, input: unknown): Promise<string> {
    const { testCode, report } = input as {
      testCode: string;
    ...
    if (report.fix.confidence < 0.8) {
      throw new Error(
        `Confidence too low (${report.fix.confidence}) — requires human review`,
      );
    }
