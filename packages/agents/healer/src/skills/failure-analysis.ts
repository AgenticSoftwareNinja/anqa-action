...
import type { AgentContext, Skill, TestResult, HealingReport } from "@agentic-nqa/core";

export const failureAnalysisSkill: Skill = {
  name: "failure-analysis",
  description: "Analyze a test failure and classify the diagnosis",

  async execute(ctx: AgentContext, input: unknown): Promise<HealingReport> {
    const { testResult, testCode } = input as {
    ...
    const pastFailures = await ctx.rag.search(
      `playwright test failure: ${errorMsg}`,
      { type: "failure", limit: 3 },
    );

    const pastContext = formatRAGContext(
      pastFailures,
...
- "suggestedFix": what to change (if diagnosis is not "app-bug")`,
    ...
    const analysis = parseLLMJson<{
      diagnosis: HealingReport["diagnosis"];
      confidence: number;
      explanation: string;
      suggestedFix?: string;
    }>(result.content);
    ...
    return {
      ...
      fix:
        analysis.diagnosis !== "app-bug" && analysis.suggestedFix
          ? {
...
function mapDiagnosisToFixType(
  diagnosis: string,
): "selector" | "wait" | "assertion" | "flow" {
  switch (diagnosis) {
