...
import { Conductor, ImprovementEngine, Scheduler, } from "@agentic-nqa/orchestrator";
...
export const improveCommand = new Command("improve")
    ...
    .option("-a, --agent <name>", "Target specific agent (playwright-planner|playwright-generator|playwright-healer)")
    .option("-t, --time-budget <hours>", "Time budget in hours (for overnight mode)", "8")
    ...
    if (options.overnight) {
        const timeBudgetMs = parseFloat(options.timeBudget) * 60 * 60 * 1000;
        console.log(`[improve] Overnight mode: ${cycles} cycles, ${options.timeBudget}h time budget`);
        ...
        console.log(`Success rate: ${result.totalExperiments > 0 ? ((result.keptExperiments / result.totalExperiments) * 100).toFixed(1) : 0}%`);
    }
