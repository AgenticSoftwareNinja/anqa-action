...
export const statusCommand = new Command('status')
    ...
    console.log('  playwright-generator  Test plan → .spec.ts files (self-verifying)');
    console.log('  playwright-healer     Auto-heal failing tests\n');
    console.log('Stack:');
    console.log('  Browser:   playwright-cli (primary) + @playwright/mcp (fallback)');
    ...
    try {
        const checkpoint = JSON.parse(await readFile('.anqa-checkpoint.json', 'utf-8'));
        console.log('Last Improvement Run:');
        console.log(`  Cycle: ${checkpoint.currentCycle}`);
        console.log(`  Saved: ${checkpoint.savedAt}`);
        console.log(`  Baseline pass rate: ${(checkpoint.baselineMetrics?.passRate * 100).toFixed(1)}%\n`);
    }
    catch {
    ...
    console.log('  anqa improve --cycles <N>     Autoresearch improvement loop');
