...
import type { TestPlan } from "@agentic-nqa/core";
...
export const generateCommand = new Command("generate")
  ...
  .action(async (options) => {
    ...
    const testPlan = JSON.parse(planContent) as TestPlan;

    console.log(
      `[generator] Plan: ${testPlan.flows.length} flows for ${testPlan.targetApp}`,
    );
    ...
    console.log(
      `[generator] Result: ${result.outputs.totalPassed}/${result.outputs.totalGenerated} passed`,
    );
    console.log(`[generator] Status: ${result.status}`);

    for (const artifact of result.artifacts) {
      const status = (artifact.metadata as Record<string, unknown>)?.passed
        ? "PASS"
        : "FAIL";
