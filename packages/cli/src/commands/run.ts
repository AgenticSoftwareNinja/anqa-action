...
export const runCommand = new Command('run')
  ...
  .action(async (options) => {
    ...
    console.log(
      `Generated: ${genResult.outputs.totalPassed}/${genResult.outputs.totalGenerated} passing\n`,
    );
    ...
    if (!options.skipHeal) {
      ...
      try {
        ...
        const healTask = {
          ...
          targetApp: { name: appName, url: options.target },
          input: {
            failedTests: parsePlaywrightReport(report).filter(
              (r) => r.status === 'failed',
            ),
          },
        };
        ...
        console.log(
          `Healed: ${healResult.outputs.healed}, App bugs: ${healResult.outputs.appBugs}\n`,
        );
      } catch {
