...
describe("createRAGClient", () => {
  describe("search", () => {
    it("passes projectId as filter_project_id to RPC", async () => {
      ...
      expect(supabase.rpc).toHaveBeenCalledWith(
        "match_knowledge",
        expect.objectContaining({ filter_project_id: "proj-abc" }),
      );
    });
    ...
    it("passes null for filter_project_id when projectId is undefined", async () => {
      ...
      expect(supabase.rpc).toHaveBeenCalledWith(
        "match_knowledge",
        expect.objectContaining({ filter_project_id: null }),
      );
    });
  });
  ...
  describe("ingest", () => {
    it("includes project_id in insert when provided", async () => {
      ...
      const insertMock = supabase.from("knowledge").insert as ReturnType<
        typeof vi.fn
      >;
