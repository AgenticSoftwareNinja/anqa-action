...
export function truncateDiff(patch, maxLines) {
    const lines = patch.split("\n");
    if (lines.length <= maxLines)
        return patch;
    return lines.slice(0, maxLines).join("\n") + "\n[truncated]";
}
export function heuristicMatch(changedFiles, index) {
    const matched = [];
    const uncertain = [];
...
async function llmEscalation(uncertainFiles, flowInventory, anthropicApiKey) {
...
export async function mapDiffToFlows(options) {
        ...
        if (elapsed < MAPPING_TIMEOUT_MS) {
            try {
                const llmResults = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error("mapping_timeout")), MAPPING_TIMEOUT_MS - elapsed)),
                ]);
