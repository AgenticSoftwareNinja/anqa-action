 ...
 */
export async function runPhaseWithBudget(name, budgetMs, fn, fallback) {
 ...
 */
export function diffFlowInventory(currentFlows, storedFlows) {
 ...
 */
export function shouldSkipNightly(repoPath) {
    try {
        const output = execFileSync("git", ["log", "--oneline", "--since=24 hours ago", "--no-merges"], { cwd: repoPath, stdio: ["pipe", "pipe", "pipe"], timeout: 10_000 });
 ...
 */
export function buildNightlyBranchName() {
    const now = new Date();
    const pad = (n) => String(n).padStart(2, "0");
    const date = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}`;
    return `anqa/nightly-${date}`;
}
//# sourceMappingURL=nightly-utils.js.map