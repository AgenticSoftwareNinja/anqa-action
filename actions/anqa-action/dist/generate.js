 ...
 */
export async function fetchAuditData(apiBaseUrl, apiKey, projectId) {
    const response = await fetch(`${apiBaseUrl}/api/action/audit?project_id=${encodeURIComponent(projectId)}`, {
 ...
 */
export async function probeAuth(targetUrl) {
    const response = await fetch(targetUrl, {
        method: "HEAD",
        signal: AbortSignal.timeout(15_000),
    });
    if (response.status === 401 || response.status === 403) {
        throw new Error(`Auth probe failed: HTTP ${response.status} — target requires authentication`);
    }
}
// ---------------------------------------------------------------------------
// Main pipeline
...
export async function runGenerate(options) {
    const { repoPath, targetUrl, anthropicApiKey, projectId, authConfig, githubToken, githubRepository, maxFlows, auditData: providedAudit, apiBaseUrl, apiKey, } = options;
    ...
    const tests = result.artifacts.map((artifact) => {
        ...
        const gap = selectedGaps.find((g) => g.flowName === flowName || g.flowId === flowName);
        return {
    ...
    if (passingTests.length > 0) {
        ...
        try {
            ...
            if (files.length > 0) {
                ...
                const coverageAfter = Math.min(100, coverageBefore +
