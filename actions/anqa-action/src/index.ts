...
function parsePRContext(): Pick<ActionConfig, "prNumber" | "prUrl" | "prBaseBranch" | "prHeadBranch" | "prHeadSha" | "prIsFork" | "prIsDraft"> {
  const defaults = { prNumber: 0, prUrl: "", prBaseBranch: "", prHeadBranch: "", prHeadSha: "", prIsFork: false, prIsDraft: false };
  const eventPath = process.env.GITHUB_EVENT_PATH;
  if (!eventPath) return defaults;
  try {
...
function readConfig(): ActionConfig {
  ...
  return {
    mode: (process.env.INPUT_MODE || "audit") as ActionConfig["mode"],
...
async function main(): Promise<void> {
  ...
  if (effectiveMode === "pr-analysis") {
    ...
    if (!lockResult.acquired) {
      ...
      await postOrUpdatePRComment(octokit, owner, repo, config.prNumber,
  ...
  try {
    ...
    if (autoGenerate) {
      const criticalHighGaps = report.gaps.filter(
