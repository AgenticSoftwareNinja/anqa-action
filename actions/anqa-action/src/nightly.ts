 ...
 */
export async function healTestSafe(
  testFilePath: string,
  error: string,
  options: NightlyOptions,
  budgetMs: number,
): Promise<NightlyHealResult> {
...
export async function runNightly(options: NightlyOptions): Promise<NightlyPayload> {
  ...
  const trigger = (process.env.GITHUB_EVENT_NAME === "schedule" ? "schedule" : "manual") as "schedule" | "manual";
  ...
  const healedTests: NightlyHealResult[] = [];
  const newTests: NightlyGenResult[] = [];
  let healingTimeMs = 0;
  let crawlTimeMs = 0;
  ...
  const failedTests: Array<{ file: string; error: string }> = [];
  let testsRun = 0;
  let testsPassed = 0;
  ...
  if (failedTests.length > 0) {
    ...
    const healResults = await runPhaseWithBudget(
      "healing",
      Math.min(DEFAULT_HEAL_BUDGET_MS, remainingBudget()),
      async (signal) => {
        const results: NightlyHealResult[] = [];
  ...
  let newFlows: FlowInventoryItem[] = [];
  ...
  if (remainingBudget() === 0) {
    ...
    if (!auditData || auditData.last_audit_age_hours > 168) {
      ...
      const crawlResult = await runPhaseWithBudget(
        "crawl",
        Math.min(DEFAULT_CRAWL_BUDGET_MS, remainingBudget()),
        async (signal) => {
          ...
          const discoveredFlows: FlowInventoryItem[] =
    ...
    if (newFlows.length > 0 && remainingBudget() > 0) {
      ...
      const genResults = await runPhaseWithBudget(
        "generation",
        Math.min(DEFAULT_GEN_BUDGET_MS, remainingBudget()),
        async (signal) => {
          const storageStatePath =
            authConfig?.method === "setup_file" && authConfig.path
              ? join(repoPath, authConfig.path)
  ...
  if (healedCount > 0 || passingNewTests.length > 0) {
    ...
    try {
      ...
      if (files.length > 0) {
        ...
        if (healedCount > 0) commitParts.push(`heal ${healedCount} tests`);
        if (passingNewTests.length > 0) commitParts.push(`add ${passingNewTests.length} new tests`);
...
function estimateNightlyCost(
  healed: NightlyHealResult[],
  generated: NightlyGenResult[],
): number {
...
function buildNightlyPRBody(
  healedTests: NightlyHealResult[],
  newTests: NightlyGenResult[],
  targetUrl: string,
): string {
