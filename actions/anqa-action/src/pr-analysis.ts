...
export async function fetchAuditDataExtended(
  apiBaseUrl: string,
  apiKey: string
): Promise<AuditDataExtended | null> {
...
export async function runPRAnalysis(
  ...
  const testResults: PRAnalysisTestResult[] = [];
  const healedFileContents = new Map<string, string>(); // full file content for direct push
  ...
  if (config.auto_commit_heals && healed > 0 && !options.prIsFork) {
    try {
      ...
      const filesToPush = Array.from(healedFileContents.entries()).map(
...
async function runSingleTest(
  ...
  try {
    execFileSync(
      "npx",
      ["playwright", "test", flow.test_file!, "--reporter=json"],
      {
        timeout: timeoutMs,
...
async function healTest(
  ...
  options: PRAnalysisOptions,
  budgetMs: number
): Promise<{ healed: boolean; attempts: number; diff: string | undefined; fullContent: string | undefined }> {
...
function computeDiff(original: string, healed: string): string {
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    if (orig !== heal) {
      ...
      if (heal !== undefined) diffLines.push(`+ ${heal}`);
    }
  }
...
function buildPayload(
  options: PRAnalysisOptions,
  tests: PRAnalysisTestResult[],
  stats: { heuristic_matches: number; llm_escalations: number; unanalyzed_files: number; index_hit_rate: number },
  filesChanged: number,
  startTime: number,
  timingMs: { mapping: number; execution: number; healing: number } = { mapping: 0, execution: 0, healing: 0 }
): PRAnalysisPayload {
